...
void loop() {
  ...
  if ((dist_raw >= _DIST_MIN) && (dist_raw <= _DIST_MAX)){
      ...
      dist_raw = samples[N-1];  // Set Higher Value
      digitalWrite(PIN_LED, 1);       // LED OFF
  } else if (dist_raw < _DIST_MIN) {
      dist_raw = samples[N-1];    // Set Lower Value
      digitalWrite(PIN_LED, 1);       // LED OFF
  ...
  dist_ema = _EMA_ALPHA * dist_raw + (1 - _EMA_ALPHA) * samples[-1];
  for(int i = 0; i < N-1; i++){
    samples[i] = samples[i+1];
    median += samples[i];
  }
  samples[N-1] = dist_ema;
  median += samples[N-1];
  median /= N;
  ...
  Serial.print(",Mid:");  Serial.print(median);
